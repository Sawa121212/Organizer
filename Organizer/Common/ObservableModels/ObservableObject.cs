...
using Organizer.Common.ObservableModels.Cheaz.Common.Prism.Attributes;
using Organizer.Infrastructure.Security.Login;
...
namespace Organizer.Common.ObservableModels
{
    ...
    public class ObservableObject : IObservableObject
    {
        ...
        [Browsable(false)]
        public Dictionary<string, List<string>> CollectionPropertyDependencies { get; private set; }
        ...
        private void VerifyPropertyName(string propertyName)
        {
                ...
                && myType.GetProperty(propertyName) == null)
            {
                ...
                if (descriptor?.GetProperties().Cast<PropertyDescriptor>().Any(property => property.Name == propertyName) == true)
                {
                    return;
                }
        ...
        private void RaisePropertyChanged<TKey, TValue>(IReadOnlyDictionary<TKey, TValue> dictionary, TKey key, params string[] propertiesToExclude) where TValue : IEnumerable<string>
        {
        ...
        {
            var sourceDictionary = (Dictionary<string, Dictionary<string, HashSet<string>>>)null;

            foreach (var propertyInfo in GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
            {
                foreach (var propertyAttribute in propertyInfo.GetCustomAttributes<ValueDependsOnExternalPropertyAttribute>(false))
                {
                    sourceDictionary = sourceDictionary ?? new Dictionary<string, Dictionary<string, HashSet<string>>>();

                    if (!sourceDictionary.TryGetValue(propertyAttribute.SourceName, out var propertyDictionary))
                    {
                        propertyDictionary = new Dictionary<string, HashSet<string>>();
                        sourceDictionary.Add(propertyAttribute.SourceName, propertyDictionary);
                        var sourceProperty = GetObservablePropertyFor(this, propertyAttribute.SourceName);
                        if (sourceProperty != null)
                        {
                            AddDependencySource(propertyAttribute.SourceName, this);
                        }
                    }

                    if (!propertyDictionary.TryGetValue(propertyAttribute.PropertyName, out var hashSet))
                    {
                        hashSet = new HashSet<string>();
                        propertyDictionary.Add(propertyAttribute.PropertyName, hashSet);
                    }
        ...
        {
            foreach (var info in sourceObject.GetType().GetProperties().Where(pi => pi.Name.Equals(propertyName, StringComparison.InvariantCultureIgnoreCase)))
            {
        ...
        {
            foreach (PropertyInfo info in sObject.GetType().GetProperties().Where(pi => pi.Name == collectionPropertyName))
            {
